...
df_bsms = pd.read_csv(filepath_or_buffer=args.trj_filename, sep = ';', header=file_fzp_start(args.trj_filename),skipinitialspace = True,nrows = 150000000,usecols=['t','v','WorldX','WorldY','Link'])
df_bsms['WorldX'] = df_bsms['WorldX'].apply(lambda x: x * 100 / 2.54 / 12 )
df_bsms['WorldY'] = df_bsms['WorldY'].apply(lambda x: x * 100 / 2.54 / 12 )
df_bsms = df_bsms.rename(columns={'Link': 'link'})
df_bsms = df_bsms.rename(columns={'WorldX': 'x'})
df_bsms = df_bsms.rename(columns={'WorldY': 'y'})
df_bsms = df_bsms.rename(columns={'t': 'localtime'})
df_bsms = df_bsms.rename(columns={'v': 'Speed'})
...
for roadway in bnlinks:
    ...
    df_bsms_toprint  = df_bsms[(df_bsms[roadway+'x'] >= 6000) & (df_bsms[roadway+'x'] <= 7000)]
    df_bsms_toprint.to_csv(path_or_buf = "/gluster/gluster1/bsm_data_emulator/GT_code/Shockwave/6000GT.csv", index=False, header=True, mode='w')
    foo
if args.out:
    out_file = dir_path + '/' + args.out

else:
    out_file = dir_path + '/highdemandincidentwithstartsxy.csv'
