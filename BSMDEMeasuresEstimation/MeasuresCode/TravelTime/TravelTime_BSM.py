...
def run_travel_time(df_bsms, routes, simulation_seconds,traveltime_output_name):
	...
	for route_group in routes:
		for route in routes[route_group]:
			...
			for start_time in range(0,int(simulation_seconds) + 150,300):
				...
				while vehicle["end_time"] is None:
					...
					for message in messages:
						...
						message_weights.append([message, 1/(time_diff + dis**2)**.5])
					message_weights = sorted(message_weights, reverse=True, key=lambda x: x[1])
