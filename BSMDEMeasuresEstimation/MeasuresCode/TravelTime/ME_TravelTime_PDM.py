...
def run_ME_traveltime_PDMs(df_pdms, routes, simulation_seconds, traveltime_output_name = 'traveltime_me_pdm.csv'):
    ...
    for route_group in routes:
        ...
        for route in routes[route_group]:
            ...
            for start_time in range(MIN_TIME,simulation_seconds+SECONDS_INTERVAL,SECONDS_INTERVAL):
...
def get_pdms(vehicle, df_pdms, time_max, distance_max, route_group, route):
    messages = []
    ...
    return get_pdms(vehicle = vehicle, df_pdms = df_pdms, time_max = time_max + TIME_WINDOW, distance_max = distance_max + DIST_WINDOW, route_group = route_group, route = route)
...
def get_start_time(vehicle, df_pdms, route_group, route):
    ...
    while (True):
        ...
        t.start('get_pdms')
        df_msgs = get_pdms(vehicle = vehicle, df_pdms = df_pdms, time_max = TIME_WINDOW, distance_max = DIST_WINDOW, route_group = route_group, route = route)
        t.stop('get_pdms')
