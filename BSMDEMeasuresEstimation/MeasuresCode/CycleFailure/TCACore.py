...
def set_logger(console_level = logging.INFO, file_level = logging.DEBUG, include_file = True, append_file = False, filename = 'tca2.log'):
...
file_path = dir_path + '/' + 'me_cyclefailure.log'
logger = set_logger(include_file = True, file_level = logging.DEBUG, filename = file_path)
...
class Timer:
   ...
   def __getitem__(self, title):
        if self.valid_title(title) and len(self.timers[title]['values']) >0:
                return title + ',' + \
   ...
   def stop(self, title=''):
        if self.enabled:
            ...
            if self.timers.has_key(title):
                self.timers[title]['values'].append(time.time() - self.timers[title]['st'])
