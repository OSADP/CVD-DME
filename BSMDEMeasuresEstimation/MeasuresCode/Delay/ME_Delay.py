...
def run_gt_delay(travel_times, full_routes, link_lengths, speed_limits, delay_output_name):
        ...
        for route_group in travel_times.keys():
            for route in travel_times[route_group].keys():
                for minute in travel_times[route_group][route].keys():
                    ...
                    average_delay = travel_time - delay_time 
...
parser = argparse.ArgumentParser(description='ME program for reading in csv files and producing Delay values')
...
if args.out:
    ...
    out_file = dir_path + '/me_delay.csv'
